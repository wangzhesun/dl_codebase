...
#         super(live_continual_seg_trainer, self).__init__(cfg, backbone_net, post_processor,
#                                                          criterion, dataset_module, device)
#
#         self.normalizer = tv.transforms.Normalize(
#             mean=self.cfg.DATASET.TRANSFORM.TRAIN.TRANSFORMS_DETAILS.NORMALIZE.mean,
...
#         np.random.seed(1234)
#         self.base_img_candidates = np.random.choice(np.arange(0, len(self.train_set)),
#                                                     replace=False, size=(memory_bank_size,))
...
#                         self.novel_adapt_single(img_chw, torch.tensor(vos_pred_map).int(),
#                                                 canonical_obj_name, scene_name + 'vos', i)
#                     # if True:
#                     #     if mask.sum() > 0:
...
#                     metrics_dict = utils.compute_binary_metrics(
#                         (pred_map == interest_cls_idx).astype(np.uint8), mask)
#                     tp_cnt += metrics_dict['tp']
#                     fp_cnt += metrics_dict['fp']
#                     tn_cnt += metrics_dict['tn']
#                     fn_cnt += metrics_dict['fn']
#                     if save_to_disk_flag:
#                         pred_prob_map_to_save = torch.softmax(pred_prob_map, dim=0).cpu().numpy()[
#                                                     -1] * 65535
...
#                         self.novel_adapt_single(img_chw, provided_mask, canonical_obj_name,
#                                                 scene_name, i)
...
#                                     if inst in annotated_frame_per_inst and len(
#                                             annotated_frame_per_inst[inst]) >= 5:
#                                         continue
#                                     # At least 300 frames between adjacent annotations
#                                     if inst in annotated_frame_per_inst and i < \
#                                             annotated_frame_per_inst[inst][-1] + 300:
#                                         continue
#                                     # IoU
#                                     if canonical_obj_name in self.class_names:
#                                         try:
...
#                                     provided_mask, num_click = my_ritm_segmenter.auto_eval(img,
#                                                                                            instance_mask,
...
#                                         provided_mask = mask
#                                     if save_to_disk_flag:
#                                         path = os.path.join(save_base_dir,
#                                                             f"{str(i).zfill(6)}_provided.npy")
#                                         np.save(path, provided_mask)
#                                     print("Spent {} clicks".format(num_click))
#                                     num_clicks_spent[scene_name] += num_click
#                                     provided_mask = torch.tensor(provided_mask).int()
#                                     self.novel_adapt_single(img_chw, provided_mask,
#                                                             canonical_obj_name, scene_name, i)
#                                     self.training_token_cnt += 1
#                                     vos_label = provided_mask.long()
#
...
#                     utils.save_to_disk(self.cfg, img_chw, os.path.join(cur_dir,
#                                                                        f'{scene_name}_{str(frame_idx).zfill(6)}.jpg'))
#                     utils.save_to_disk(self.cfg, (mask_hw * 255).long(), os.path.join(cur_dir,
#                                                                                       f'{scene_name}_{str(frame_idx).zfill(6)}.png'))
#         inter = None
#         union = None
...
#                 my_inter, my_union = utils.compute_iu(pred_map, elevated_mask,
...
#                     if k not in self.learned_data_map or len(self.psuedo_database[k]) > len(
#                             self.learned_data_map[k]):
#                         self.data_lock.acquire()
...
#             self.psuedo_database[obj_name] = [
#                 (img_chw, mask_hw, img_roi, mask_roi, scene_name, frame_idx)]
#             new_clf_weights = self.classifier_weight_imprinting_one(img_chw, mask_hw)
#             self.post_processor.pixel_classifier.class_mat.weight = torch.nn.Parameter(
#                 new_clf_weights)
#             self.cls_name_id_map[obj_name] = num_existing_class  # 0-indexed shift 1
#         else:
#             self.psuedo_database[obj_name].append(
#                 (img_chw, mask_hw, img_roi, mask_roi, scene_name, frame_idx))
#         self.data_lock.release()
#         self.model_lock.release()
#
...
#             class_weight_vec = utils.masked_average_pooling(supp_mask_bhw_tensor == 1,
...
#                 syn_img_chw, syn_mask_hw = utils.copy_and_paste(img_roi, mask_roi, syn_img_chw,
#                                                                 syn_mask_hw, selected_class_id)
#
...
#                         img_chw, mask_hw, _, _, scene_name, frame_idx = random.choice(
#                             self.psuedo_database[novel_obj_name])
#                         mib_sample_weight.append(sample_weight_lookup(scene_name, frame_idx))
#                         # TODO: implement proper augmentation
#                         img_chw = tr_F.pad(img_chw, [(512 - img_chw.shape[2]) // 2,
#                                                      (512 - img_chw.shape[1]) // 2])
#                         mask_hw = tr_F.pad(mask_hw, [(512 - mask_hw.shape[1]) // 2,
#                                                      (512 - mask_hw.shape[0]) // 2])
#                         image_list.append(img_chw)
#                         mask_list.append(mask_hw)
...
#                             bg_prob_chw = output_logit_bchw[
#                                 b, torch.arange(output_logit_bchw.shape[1]) != partial_positive_idx[
#                                     b]]
...
class live_continual_seg_trainer(seg_trainer):
    ...
    def test_one(self, device):
        ...
        print("Base IoU after adaptation")
        print(np.mean(class_iou[:self.cfg.num_classes]))
        save_dir = '/root/autodl-tmp/data/scannet/provided_masks_everything'
        if save_to_disk_flag:
            for k in self.psuedo_database:
                cur_dir = os.path.join(save_dir, k)
                os.makedirs(cur_dir, exist_ok=True)
    ...
    def monitor_thread(self):
        ...
        while True:
            ...
            if self.training_token_cnt >= 1:
                for k in self.psuedo_database:
                    # If not fine-tuned or new data is available...
                    if k not in self.clean_data_map or len(self.psuedo_database[k]) > len(
                            self.clean_data_map[k]):
                        self.data_lock.acquire()
                        self.fine_tune_busy_flag = True
                        ...
                        print("Total training data pool size: {}".format(
                            len(cur_adaptation_data_record)))
            ...
            for iter_i in t:
                ...
                for _ in range(batch_size):
                    if torch.rand(1) < 0.8:
                        ...
                        # partially-labeled image
                        img_chw, mask_hw, _, _, scene_name, frame_idx = random.choice(
                            self.clean_data_map[novel_obj_name])
                        mib_sample_weight.append(sample_weight_lookup(scene_name, frame_idx))
                        # TODO: implement proper augmentation
                        img_chw = tr_F.pad(img_chw, [(512 - img_chw.shape[2]) // 2,
                                                     (512 - img_chw.shape[1]) // 2])
